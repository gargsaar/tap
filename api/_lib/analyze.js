// Open-source LLM analysis (Llama 3.3 70B on Groq) → insights.json:
// summary + key points + decisions + meeting minutes + action items.
//
...
//
// runAnalysis() is shared by /api/analyze (manual re-run) and the Deepgram
// callback (automatic, right after transcription).
import { getEntry, upsertEntry, saveInsights, fetchText } from "./store.js";
...
export async function runAnalysis(id) {
  if (!process.env.GROQ_API_KEY) throw new Error("GROQ_API_KEY is not set in Vercel env vars");

  const entry = await getEntry(id);
  if (!entry) throw new Error(`no recording ${id}`);
  if (!entry.transcriptUrl) throw new Error("recording has no transcript yet");
